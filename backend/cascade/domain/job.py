...
class SimulationJob:
    ...
    Attributes:
        ...
        working_dir:  Where the backend stages input/output files.
                      Set by JobService before submit() is called.
        ...
        notes:        Optional human-readable label (e.g. "rod_pos=24cm").
    """
    id:           str
    ...
    def input_dir(self) -> Path:
        """Return the input files directory for this job.

        Raises:
            RuntimeError: If working_dir has not been set yet.
        """
    ...
    def output_dir(self) -> Path:
        """Return the output files directory for this job."""
        if self.working_dir is None:
            raise RuntimeError(
                f"Job '{self.id}' has no working_dir set."
            )
